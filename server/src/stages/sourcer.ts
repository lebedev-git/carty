...
function buildBlockPrompt(
  ...
  const intro = grounded
    ? `Ниже по каждому кластеру даны РЕАЛЬНЫЕ результаты веб-поиска (новости за последние 3 года). Для КАЖДОГО кластера извлеки ${casesPerCell} проблемных новостных кейса, опираясь строго на эти результаты (только реальные события, не общие статьи!), и в "source" поставь точный URL соответствующего результата.`
